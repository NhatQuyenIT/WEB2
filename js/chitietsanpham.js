...
function addChiTietToWeb(p) {
    ...
    // Đổi title
    document.title = p.TenSP + ' - Thế giới - Tai nghe';
    ...
    var s = addThongSo('Thời lượng pin', p.ThoiLuongPin);
    s += addThongSo('Cổng sạc', p.CongSac);
    s += addThongSo('Tương thích', p.TuongThich);
    s += addThongSo('Kết nối cùng lúc', p.KetNoiCungLuc);
    s += addThongSo('Công nghệ kết nối', p.CongNgheKetNoi);
    s += addThongSo('Điều khiển', p.DieuKhien);
    s += addThongSo('Kích thước', p.KichThuoc);
    s += addThongSo('Khối lượng', p.KhoiLuong);
    s += addThongSo('Sản xuất tại', p.SanXuatTai);
    info.innerHTML = s;
