...
function copyObject(o) {
  if (typeof o !== 'undefined') {
    ...
    return null; // Hoặc giá trị khác tùy theo nhu cầu của bạn
  }
}
...
function getCurrentAdmin(onSuccess, onFail) {
  $.ajax({
    type: "POST",
    url: "php/xulytaikhoan.php",
    dataType: "json",
...
function checkDangKy() {
  ...
  var checkpass = document.getElementById("re-Pass").value;
...
if (pass !== checkpass) {
  ...
  // Dừng việc thực hiện đăng ký và không gửi request AJAX
  return false;
}

  // Nếu mật khẩu nhập lại khớp với mật khẩu đã nhập trước đó, tiếp tục thực hiện đăng ký bằng AJAX
  $.ajax({
    url: "php/xulytaikhoan.php",
    type: "post",
    dataType: "json",
    ...
    s.innerHTML = `
			(function () {
				...
				for (var li of li_s) {
					...
					switch (loai) {
						case "Thời lượng pin:":
							result.detail.pin = giatri.replace('"', "'");
							break;
						case "Cổng sạc:":
							result.detail.port = giatri;
							break;
						case "Tương thích:":
							result.detail.compatible = giatri;
							break;
						case "Kết nối cùng lúc:":
							result.detail.connect = giatri;
							break;
						case "Công nghệ kết nối:":
							result.detail.cpu = technology;
							break;
						case "Điều khiển:":
							result.detail.conduct = giatri;
							break;
						case "Kích thước:":
							result.detail.size = giatri;
							break;
						case "Khối lượng:":
							result.detail.volume = giatri;
							break;
						case "Sản xuất tại:":
							result.detail.produce = giatri;
							break;
					}
				}
